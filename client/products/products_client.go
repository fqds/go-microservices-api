...
type ClientService interface {
	...
	ListSingleProducts(params *ListSingleProductsParams, opts ...ClientOption) (*ListSingleProductsOK, error)
...
/*
ListSingleProducts Return a list of products from the database
*/
func (a *Client) ListSingleProducts(params *ListSingleProductsParams, opts ...ClientOption) (*ListSingleProductsOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewListSingleProductsParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "listSingleProducts",
		Method:             "GET",
		PathPattern:        "/products/{id}",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &ListSingleProductsReader{formats: a.formats},
		Context:            params.Context,
		Client:             params.HTTPClient,
	}
	for _, opt := range opts {
	...
	success, ok := result.(*ListSingleProductsOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for listSingleProducts: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
